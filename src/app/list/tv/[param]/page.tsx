...
import TvPosterCard from "@/components/sections/TV/Cards/Poster";
...
export default function TvListPage({ params }: { params: { param: string } }) {
  const { param } = params;
  const listConfig = siteConfig.queryLists.tvShows.find(t => t.param === param);
    ...
    <div className="flex flex-col gap-10 min-h-screen p-4 md:p-8 max-w-7xl mx-auto pt-24">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">Explore top TV shows</p>
        </div>
      </div>
          ...
          <div className="movie-grid w-full">
            {data.pages.map((page) => {
              return page.results.map((tv) => {
                return <TvPosterCard key={tv.id} tv={tv} variant="bordered" />;
              });
            })}
          </div>

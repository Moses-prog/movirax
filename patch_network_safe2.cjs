const fs = require("fs");
const lines = fs.readFileSync("src/components/NetworkStatus.tsx", "utf8").split("\n");

const startIndex = lines.findIndex(l => l.includes("if (!isOffline) return null;"));
...
if (startIndex !== -1) {
  ...
  lines.splice(startIndex + 1, lines.length - startIndex, newReturn);
  fs.writeFileSync("src/components/NetworkStatus.tsx", lines.join("\n"));
  console.log("Replaced using line index successfully!");
} else {
